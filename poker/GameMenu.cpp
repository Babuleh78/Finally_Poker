#include "GameMenu.h"
void game::GameMenu::SetTextOpt(sf::Text& text, sf::String& str, float xpos, float ypos) {
	text.setFont(font);
...
{
	if (!font.loadFromFile("font/Inky-Thin-Pixels_0.ttf")) exit(32);
	max_menu = name->getSize() - 1;
	mainMenu = new sf::Text[name->getSize()];
	for (int i = 0, ypos = menu_Y; i < max_menu; i++, ypos += menu_Step) SetTextOpt(mainMenu[i], name[i], menu_X, ypos);
	mainMenuSelected = 0;
